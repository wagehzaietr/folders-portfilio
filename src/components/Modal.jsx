...
const Modal = ({ handleClick, isdark, isOpen, onClose, initialPage = 'projects' }) => {
  // Memoize sounds to prevent re-creation on re-renders
  const [popClose] = useSound('/closed.wav', { volume: 0.5 })
  const [clickSound] = useSound('/pop.ogg', { volume: 0.3 })
  const [clickSwitch] = useSound('/clicksound.mp3', { volume: 0.3 })
  
  const [selectedProject, setSelectedProject] = useState(null)
  const [currentPage, setCurrentPage] = useState(initialPage)

  // Memoize page variants to prevent re-creation
  ...
  const theme = useMemo(() => ({
    ...
    card: isdark ? 'bg-gray-800 hover:bg-gray-750' : 'bg-gray-50 hover:bg-gray-100',
    button: isdark ? 'bg-gray-700 hover:bg-gray-650' : 'bg-gray-100 hover:bg-gray-200',
    primaryButton: isdark ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-500 hover:bg-blue-600',
    secondaryText: isdark ? 'text-gray-400' : 'text-gray-600',
    mutedText: isdark ? 'text-gray-500' : 'text-gray-400'
  }), [isdark])

  // Memoize skills data with icons
  const enhancedSkillsData = useMemo(() => skillsData.map(skill => {
    const icons = {
  ...
  // Form fields data
  const formFields = useMemo(() => ['Name', 'Email', 'Message'], [])

  // Optimized modal content to reduce re-renders
        ...
        >
          <div className='flex flex-col md:flex-row gap-6 mb-8'>
            <motion.div
              ...
              className={`rounded-xl overflow-hidden ${isdark ? 'bg-gray-800' : 'bg-gray-100'} flex-shrink-0`}
            >
              <img
                ...
                >
                  {selectedProject.tags.map((tech, index) => (
                    <motion.span
                      ...
                      whileTap={{ scale: 0.95 }}
                      className={`px-3 py-1 rounded-full text-sm font-medium ${isdark ? 'bg-gray-700 text-blue-400' : 'bg-blue-50 text-blue-600'}`}
                    >
                      {tech}
                    </motion.span>
                  ))}
        ...
        return (
          ...
          >
            ...
            <div className='mt-auto pt-4 border-t border-gray-100'>
              <h3 className={`text-sm font-medium uppercase mb-3 ${theme.mutedText}`}>
                Connect With Me
              </h3>
              ...
              <div className='space-y-4'>
                {formFields.map((field, index) => (
                  ...
                  >
                    ...
                    {field === 'Message' ? (
                      <textarea
                        className={`w-full px-4 py-2 rounded-lg border ${theme.border} ${isdark ? 'bg-gray-800' : 'bg-white'}`}
                        rows='4'
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-2 rounded-lg border ${theme.border} ${isdark ? 'bg-gray-800' : 'bg-white'}`}
                        placeholder={field === 'Email' ? 'your@email.com' : `Your ${field.toLowerCase()}`}
                      />
                    )}
                  </motion.div>
                ))}
  ...
  }, [currentPage, selectedProject, theme, isdark, enhancedSkillsData, socialLinks, formFields, pageVariants, clickSound, popClose, handleNavigation])
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className={`fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm ${isdark ? 'bg-black/70' : 'bg-black/40'}`}
          onClick={handleClose}
        >
          <motion.div
            ...
            style={{
              boxShadow: isdark ? '0 30px 60px rgba(0, 0, 0, 0.4)' : '0 30px 60px rgba(0, 0, 0, 0.15)'
            }}
          >
            {/* Title bar */}
            <div className={`flex items-center justify-between p-4 ${isdark ? 'bg-gray-800' : 'bg-gray-100'} border-b ${theme.border}`}>
            ...
            {/* Content Area */}
            <div className={`p-6 min-h-[400px] max-h-[70vh] overflow-y-auto ${theme.background}`}>
              {modalContent}
            </div>

            {/* Status bar */}
            <div className={`px-4 py-2 text-xs ${isdark ? 'bg-gray-800 text-gray-400' : 'bg-gray-100 text-gray-500'} border-t ${theme.border} flex justify-between items-center`}>
              <div>
                {currentPage === 'projects' && !selectedProject && `${projectData.length} projects`}
                {currentPage === 'about' && `${enhancedSkillsData.length} skills`}
              </div>
